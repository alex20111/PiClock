...
public class CmdTranslator {
	...
	private RadioScan scan = RadioScan.NONE;
	...
	public String generateScanFM(){
		scan = RadioScan.STARTED;
		stations = new ArrayList<>();
		StringBuilder cmd = new StringBuilder();
		cmd.append(START_CHAR);
		cmd.append(Command.SCAN_RADIO.getCmd());
		cmd.append(STOP_CHAR);		
	...
	private Command populateFromCommand(String receivedCmd){
		...
		}else if (cmd == Command.SCAN_RADIO) {
			String data = receivedCmd.substring(2,  receivedCmd.indexOf(">"));
			if (data.contains("end")) {
				scan = RadioScan.FINISHED;
			}else {
				stations.add(data);
			}
	...
	public RadioScan getRadioScanStatus() {
		return scan;
	}
	protected void setScan(RadioScan rd) {
		scan = rd;
	}
